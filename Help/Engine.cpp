	...
	, m_rock(12, 12, 64.0f, 64.0f)
{
	...
	// Sets up grid position for the rock boundries.
	gridPosition.x = 0.5f*resolution.x - m_rock.CELL_WIDTH  * 0.5f *(float)m_rock.GRID_SIZE_X;
	gridPosition.y = 0.5f*resolution.y - m_rock.CELL_HEIGHT * 0.5f *(float)m_rock.GRID_SIZE_Y;
	m_rock.SetPosition(gridPosition);
	// Fill the rest of our grid with dirt
	for (int x = 0; x < m_rock.GRID_SIZE_X; ++x)
	{
		for (int y = 0; y < m_rock.GRID_SIZE_Y; ++y)
		{
			m_rock.SetObject(x, y, new GridSprite(TextureHolder::GetTexture("graphics/rock.png"), GridObject::DIRT));
		}
	}
}
